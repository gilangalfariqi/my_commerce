        ...
        <form method="POST" action="{{ route('admin.categories.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 resize-none">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Category Image</label>
