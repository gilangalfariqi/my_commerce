<x-frontend-layout>
    <div x-data="whatsappFastCheckoutPage()" class="mt-4 max-w-5xl mx-auto">
        ...
        <div class="mb-6">
            <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 text-xs text-slate-400 hover:text-primary-500 transition-colors">
            ...
            <div class="lg:col-span-8">
                    ...
                    <div class="mt-4 divide-y divide-slate-800">
                        @forelse($cart->items as $item)
                            <div class="py-4 flex gap-4 items-start">
                                <div class="w-20 h-20 rounded-2xl border border-slate-800 bg-slate-900/40 overflow-hidden flex-shrink-0">
                                    <img
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                                        ...
                                        <div class="flex items-baseline sm:flex-col sm:items-end gap-1.5 sm:gap-0 mt-1 sm:mt-0">
                                            <p class="text-sm font-extrabold text-primary-500">Rp {{ number_format((float) $item->getTotalPrice(), 0, ',', '.') }}</p>
                        ...
                        @empty
                            <div class="py-14 text-center">
                                ...
                                <a href="{{ route('products.index') }}" class="mt-3 inline-block text-xs font-bold text-primary-500 hover:underline">Mulai belanja</a>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="mt-6 bg-slate-950/70 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-premium">
                    ...
                    <label class="mt-4 flex items-start gap-3 cursor-pointer select-none">
                        <input type="checkbox" x-model="confirmed" class="mt-1 w-5 h-5 accent-primary-500" />
                    ...
                    <div class="mt-5 flex items-center gap-3">
                        <button
                            ...
                            x-bind:class="confirmed ? 'bg-primary-600 hover:bg-primary-700 text-white' : 'bg-slate-800 text-slate-500'"
            ...
            <div class="lg:col-span-4">
                <div class="bg-slate-950/70 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-premium">
                    ...
                    <div class="mt-4 space-y-3 text-sm">
                        ...
                        <div class="flex justify-between text-slate-400">
                            <span>Total</span>
                            <span class="text-primary-500 font-extrabold">Rp {{ number_format((float) $cart->getGrandTotal(), 0, ',', '.') }}</span>
                        </div>
                    </div>
